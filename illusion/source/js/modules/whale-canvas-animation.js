...
export default class WhaleScene {
  ...
  updateWhaleSizing() {
    this.bodyWidth = Math.round(880 * wFactor);
    this.bodyHeight = Math.round(this.bodyWidth * 295 / 783);
    this.bodyL = Math.round((winW - this.bodyWidth) / 2);
    this.bodyT = Math.round((winH - this.bodyHeight) / 2 + 17 * wFactor);

    this.finWidth = Math.round(200 * wFactor);
    this.finHeight = Math.round(this.finWidth * 133 / 216);
    this.finL = Math.round(this.bodyL + 310 * wFactor);
    this.finT = Math.round(this.bodyT + 200 * wFactor);

    this.tailWidth = Math.round(119 * wFactor);
    this.tailHeight = Math.round(this.tailWidth * 79 / 119);
    this.tailL = Math.round(this.bodyL + this.bodyWidth - 145 * wFactor);
    this.tailT = Math.round(this.bodyT + 35 * wFactor);
  }
  ...
  updateBalloonsSizing() {
    this.balloonLeftWidth = Math.round(149 * wFactor);
    this.balloonLeftHeight = Math.round(this.balloonLeftWidth * 162 / 149);
    this.balloonLeftL = Math.round(this.bodyL + 140 * wFactor);
    this.balloonLeftT = Math.round(this.bodyT - 145 * wFactor);

    this.balloonRightWidth = Math.round(152 * wFactor);
    this.balloonRightHeight = Math.round(this.balloonRightWidth * 182 / 152);
    this.balloonRightL = Math.round(this.bodyL + 208 * wFactor);
    this.balloonRightT = Math.round(this.bodyT - 185 * wFactor);
  }
  ...
  drawThreads() {
    ...
    const threadLeftBBX = this.bodyL + 370 * wFactor;
    const threadLeftBBY = this.bodyT + 222 * wFactor;
    const threadLeftMX  = threadLeftBBX - 84 * wFactor;
    const threadLeftMY  = threadLeftBBY - 98 * wFactor;
    const threadLeftBTX = threadLeftMX + 43 * wFactor;
    const threadLeftBTY = threadLeftMY + 35 * wFactor;
    const threadLeftBMX = threadLeftMX + 84 * wFactor;
    const threadLeftBMY = threadLeftMY + 92 * wFactor;
    const threadLeftT   = threadLeftMY - 110 * wFactor;
    const threadLeftL   = threadLeftMX - 68 * wFactor;
    const threadLeftLTY = threadLeftT + 75 * wFactor;
    const threadLeftLTX = threadLeftL + 25 * wFactor;
    ...
    // draw thread right
    const threadRightBBX = this.bodyL + 312 * wFactor;
    const threadRightBBY = this.bodyT + 234 * wFactor;
    const threadRightL   = threadRightBBX - 30 * wFactor;
    const threadRightT   = threadRightBBY - 262 * wFactor;
    const threadRightLTX = threadRightL + 16 * wFactor;
    const threadRightLTY = threadRightT + 39 * wFactor;
    const threadRightMX  = threadRightL + 11 * wFactor;
    const threadRightMY  = threadRightT + 117 * wFactor;
    const threadRightBTX = threadRightL + 6 * wFactor;
    const threadRightBTY = threadRightT + 190 * wFactor;
    const threadRightBMX = threadRightL + 25 * wFactor;
    const threadRightBMY = threadRightT + 256 * wFactor;
  ...
  drawScene() {
    ...
    if (this.isAnimated) {
      const moonX = this.moonEndX - this.moonRadius * ((-this.moonRotateAngle - 32) * Math.PI / 180);
